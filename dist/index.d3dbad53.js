...
function checkDirection() {
    ...
    if (Math.abs(difX) > Math.abs(difY)) {
        if (touchendX < touchstartX) /*left*/ return;
        if (touchendX > touchstartX) /*right*/ return;
    } else {
        if (touchendY < touchstartY) /*up*/ //   console.log(run);
        document.location.href = "../run.html";
        if (touchendY > touchstartY) /*down*/ //   console.log(tri);
...
document.addEventListener("touchend", (e)=>{
    ...
    if (e.target === tri || e.target === run) checkDirection();
    return;
});
